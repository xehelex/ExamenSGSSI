...
// Estructura para guardar historial de cambios
typedef struct {
    char vieja;
    char nueva;
} Cambio;
    ...
    while ((c = fgetc(entrada)) != EOF) {
        ...
        while (i < 26 && encontrado == 0) {
            if (letra == pares[i].original) {
                // Si el reemplazo está en mayúscula, lo mantenemos
...
void ajustarSustituciones(char *origen, char *destino, ParLetra pares[]) {
    char opcion;
    Cambio historial[100]; // Máximo 100 cambios
    int totalCambios = 0;
    ...
    while (tolower(opcion) == 's') {
        ...
        while (i < 26) {
            if (tolower(vieja) == pares[i].reemplazo) {
                pares[i].reemplazo = toupper(nueva); // ahora en MAYÚSCULAS
                // Guardar en historial
                historial[totalCambios].vieja = tolower(vieja);
                historial[totalCambios].nueva = toupper(nueva);
                totalCambios++;
                break;
            }
            i++;
    ...
    if (totalCambios > 0) {
        ...
        for (int i = 0; i < totalCambios; i++) {
            printf("%c -> %c\n", historial[i].vieja, historial[i].nueva);
        }
    } else {
